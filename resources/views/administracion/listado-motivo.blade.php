...
<h2>LISTADO DE MOTIVOS DE PAUSAR UNA SOLICITUD <button id="add" class="button icono" title="Añadir Motivo"> <i class="fa-solid fa-plus" style="font-size:25px;"></i></button>
</h2>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr data-id="{{ $motivo->id }}">
                    <td>{{ $motivo->id }}</td>
                    <td class="nombre">
                            <span class="nombre-campana">{{ $motivo->nombre }}</span>
                            <form class="form-editar" style="display: none;" action="{{ route('guardar.editar.motivos',$motivo->id)}}" method="POST">
                                @csrf
                                <input type="text" name="nombre" value="{{ $motivo->nombre }}" required><br>
                        ...
                        <td class="acciones">
                            <button type="button" class="editar icono" title="Editar Motivo"><i class="fa-solid fa-pen-to-square"></i></button>
                            @if(Auth::user()->rol == "admin")
                            |&nbsp;
                            <form action="/motivosdel/{{ $motivo->id }}" method="POST" style="display: inline;">
                                @csrf
                                @method('delete')
                                <button type="submit" class="icono borrar" title="Eliminar Motivo"><i class="fa-solid fa-trash"></i></button>
                            </form>
                            @endif
                        </td>
        ...
        @if ($motivos->isEmpty())
            <p class="notickets">No hay Motivos de Pausa de solicitudes para mostrar</p>
        @endif
    </div>
<div id="modal-asignar" style="display: none;">
    <div class="modal-content">

        <form id="form-add-campana" method="POST" action="{{ route('guardar.add.motivos') }}">
            @csrf
            <label for="campana-select">Motivo de Pausa:</label>
