...
<h2>LISTADO DE SEDES <button id="add" class="button icono" title="Añadir Sede"> <i class="fa-solid fa-plus" style="font-size:25px;"></i></button>
</h2>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr data-id="{{ $sede->id }}">
                    <td>{{ $sede->id }}</td>
                    <td class="nombre">
                            <span class="nombre-campana">{{ $sede->nombre }}</span>
                            <form class="form-editar" style="display: none;" action="{{ route('guardar.editar.sedes',$sede->id)}}" method="POST">
                                @csrf
                                <input type="text" name="nombre" value="{{ $sede->nombre }}" required><br>
                        ...
                        <td class="acciones">
                            <button type="button" class="editar icono" title="Editar Sede"><i class="fa-solid fa-pen-to-square"></i></button>
                            @if(Auth::user()->rol == "admin")
                            |&nbsp;
                            <form action="/sedesdel/{{ $sede->id }}" method="POST" style="display: inline;">
                                @csrf
                                @method('delete')
                                <button type="submit" class="icono borrar" title="Eliminar Sede"><i class="fa-solid fa-trash"></i></button>
                            </form>
                            @endif
                        </td>
        ...
        @if ($sedes->isEmpty())
            <p class="notickets">No hay Sedes para mostrar</p>
        @endif
    </div>
<div id="modal-asignar" style="display: none;">
    <div class="modal-content">

        <form id="form-add-campana" method="POST" action="{{ route('guardar.add.sedes') }}">
            @csrf
            <label for="campana-select">Sede:</label>
