...
<h2>LISTADO DE TIPOS DE SOLICITUDES <button id="add" class="button icono" title="Añadir Tipo"> <i class="fa-solid fa-plus" style="font-size:25px;"></i></button>
</h2>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr data-id="{{ $tipo->id }}">
                    <td>{{ $tipo->id }}</td>
                    <td class="nombre">
                            <span class="nombre-campana">{{ $tipo->nombre }}</span>
                            <form class="form-editar" style="display: none;" action="{{ route('guardar.editar.tipos',$tipo->id)}}" method="POST">
                                @csrf
                                <input type="text" name="nombre" value="{{ $tipo->nombre }}" required><br>
                        ...
                        <td class="acciones">
                            <button type="button" class="editar icono" title="Editar Tipo"><i class="fa-solid fa-pen-to-square"></i></button>
                            @if(Auth::user()->rol == "admin")
                            |&nbsp;
                            <form action="/tiposdel/{{ $tipo->id }}" method="POST" style="display: inline;">
                                @csrf
                                @method('delete')
                                <button type="submit" class="icono borrar" title="Eliminar Tipo"><i class="fa-solid fa-trash"></i></button>
                            </form>
                            @endif
                        </td>
        ...
        @if ($tipos->isEmpty())
            <p class="notickets">No hay Tipos de Solicitudes para mostrar</p>
        @endif
    </div>
<div id="modal-asignar" style="display: none;">
    <div class="modal-content">

        <form id="form-add-campana" method="POST" action="{{ route('guardar.add.tipos') }}">
            @csrf
            <label for="campana-select">Tipo de solicitud:</label>
